...
export async function createComment(token: string, comment: Partial<UserComment>)
{
	...
	try
	{
		let response = await axios.post(`${Constants.API_URL}/comments`, comment, {
			headers: {
...
export async function updateComment(token: string, commentId: number, comment: Partial<UserComment>)
{
	...
	try
	{
		let response = await axios.put(`${Constants.API_URL}/comments/${commentId}`, comment, {
			headers: {
				"Content-Type": "application/json",
				"Authorization": `Bearer ${token}`,
			},
...
export async function deleteComment(token: string, commentId: number)
{
	...
	try
	{
		let response = await axios.delete(`${Constants.API_URL}/comments/${commentId}`, {
			headers: {
				"Content-Type": "application/json",
				"Authorization": `Bearer ${token}`,
			},
